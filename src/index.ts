...
const getTrackInfo = async (trackId: string, proxy?: boolean): Promise<trackData | undefined> => {
    try {
        const url = proxy ? `https://corsproxy.org/?${encodeURIComponent(`https://open.spotify.com/embed/track/${trackId}`)}` : `https://open.spotify.com/embed/track/${trackId}`
